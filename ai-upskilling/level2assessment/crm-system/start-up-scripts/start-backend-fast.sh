...
echo -e "${BLUE}========================================"
echo "   CRM System Backend Startup Script (Fast)"
echo "========================================"
echo -e "${NC}"
...
echo "[4/6] Building Maven projects (sequential)..."
...
# Check if Docker images exist and build only if needed
...
for service in "${docker_services[@]}"; do
    if docker images | grep -q "crm-system-$service"; then
        print_status "$service Docker image exists (skipping build)"
    else
        print_warning "$service Docker image not found (will build)"
...
if [ ${#images_to_build[@]} -gt 0 ]; then
    ...
    print_status "All Docker images exist (skipping builds)"
fi

print_status "Docker images ready"

# Start infrastructure services
echo "[6/6] Starting infrastructure services..."
